  ...
  protected submit(): void {
    ...
    this.service
      ...
      .subscribe({
        ...
        error: err => {
          this.sending.set(false);
          this.error.set(sendFailureMessage(err));
        }
      });
  }
}
...
/**
 * הטופס הזה פונה לשרת בלי משתמש מחובר, כך שכל תקלה מגיעה לפונה עצמו ולא ליומן שגיאות.
 * לכן כל מצב מקבל הודעה שמסבירה מה קרה ומה אפשר לעשות, במקום כישלון גנרי אחד.
 */
function sendFailureMessage(err: { status?: number } | undefined): string {
  switch (err?.status) {
    // status 0 = הבקשה לא הגיעה לשרת בכלל: אין רשת, השרת ישן, או שה-origin נחסם ב-CORS
    case 0:
      return 'לא הצלחנו להגיע לשרת. בדקו את החיבור לאינטרנט ונסו שוב בעוד רגע.';
    case 404:
      return 'הפרופיל הזה לא זמין כרגע לקבלת פניות.';
    case 429:
      return 'נשלחו יותר מדי פניות מהכתובת הזו. נסו שוב בעוד כמה דקות.';
    case 400:
      return extractErrorMessage(err, 'חלק מהפרטים לא תקינים. בדקו ונסו שוב.');
    default:
      // מספר הסטטוס נשאר בהודעה כדי שאפשר יהיה לדווח עליו בלי לפתוח כלי פיתוח
      return `שליחת הפנייה נכשלה (שגיאה ${err?.status ?? 'לא ידועה'}). אפשר ליצור קשר ישירות בפרטים שבעמוד.`;
  }
}
