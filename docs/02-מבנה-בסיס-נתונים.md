...
## הקשחות שיושמו במימוש (שלב 5.2)

לאחר סקירה אדוורסרית רב-סוכנית של שכבת הנתונים, נוספו ההגנות הבאות:

### בידוד רב-דיירות — שלוש שכבות
1. **קריאה** — Global Query Filter (`TenantId == currentTenant`) על כל ישות `ITenantScoped`. ברירת מחדל fail-closed: ללא דייר → אפס שורות.
2. **כתיבה** — `SaveChanges`/`SaveChangesAsync` חותם `TenantId` על ישויות חדשות, ומונע כתיבה/עדכון חוצי-דייר ושינוי `TenantId` קיים.
3. **מסד** — מפתחות זרים **מורכבים** `(EntityId, TenantId)` (עם מפתחות חלופיים על העקרונות) הופכים קישור חוצה-דייר לבלתי-אפשרי פיזית:
   - `Lesson(StudentId, TenantId) → Student`
   - `Note(StudentId, TenantId) → Student`
   - `StudentParent(StudentId, TenantId) → Student`, `(ParentId, TenantId) → Parent`
   - `Payment(ParentId, TenantId) → Parent`
   - `LessonChangeRequest(LessonId, TenantId) → Lesson`, `(RequestedByParentId, TenantId) → Parent`
   - הפניות אופציונליות (`Lesson.RequestedByParentId`, `Lesson.PaymentId`, `Note.LessonId`) נשארות חד-עמודה עם `SetNull`, ובידודן נאכף ב-`SaveChanges`.

### CHECK constraints
- `Lessons.Amount >= 0`, `Payments.Amount >= 0`, `Teachers.DefaultPricePerLesson >= 0`
- `Lessons.EndTime > StartTime`
- `LessonChangeRequests`: `ProposedEndTime > ProposedStartTime` (כששניהם קיימים)

### אינדקסים וייחודיות
- ייחודי: `TeacherSubject(TeacherId, Name)`, `Parent(TenantId, Email)`
- אינדקס חלקי לנתיב "תשלומים פתוחים": `Lesson(TenantId, PaymentRequired) WHERE PaymentId IS NULL`

### הערות
- שדות טקסט חופשי ממומשים כ-`varchar(N)` עם תקרות נדיבות (Bio/Rules 2000–4000, Content 4000 וכו') — חסם מפני שימוש לרעה.
- **לתיעוד לקראת 5.3:** מימוש `ICurrentTenant` אמיתי מתוך JWT, הוספת Authentication ל-API, ו-`RequireUniqueEmail` ל-Identity.

> מצב: מיגרציה `InitialCreate` הוחלה על PostgreSQL — 16 טבלאות, 5 CHECK constraints, 7 FKs מורכבים. ✅
